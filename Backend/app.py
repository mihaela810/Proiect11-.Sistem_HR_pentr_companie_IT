...
def get_angajati_filtrati():
    try:
        ...
        if departament:
            query += " AND id_departament = %s"; params.append(departament)
        if pozitie:
            query += " AND pozitie LIKE %s"; params.append(f"%{pozitie}%")
        if status_activ is not None:
            query += " AND activ = %s"; params.append(int(status_activ))

        order_map = {
            'nume_asc':    " ORDER BY nume ASC, prenume ASC",
            'nume_desc':   " ORDER BY nume DESC, prenume DESC",
            'departament': " ORDER BY id_departament ASC",
            'pozitie':     " ORDER BY pozitie ASC",
        }
        query += order_map.get(sortare, " ORDER BY nume ASC")
...
def schimbare_parola():
    date          = request.get_json()
    id_utilizator = date.get('id_angajat')
    parola_veche  = date.get('parola_veche')
    parola_noua   = date.get('parola_noua')

    if not id_utilizator or not parola_veche or not parola_noua:
        return jsonify({"status": "eroare", "mesaj": "Lipsesc date esentiale (id_angajat, parola_veche sau parola_noua)."}), 400
    try:
        conn   = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('proc_schimbare_parola', [int(id_utilizator), str(parola_veche), str(parola_noua)])
        conn.commit()
        return jsonify({"status": "succes", "mesaj": "Parola modificata cu succes."}), 200
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        if 'conn' in locals(): conn.close()
...
def dezactivare_cont():
    ...
    if id_angajat is None:
        return jsonify({"status": "eroare", "mesaj": "Lipseste id_angajat."}), 400
    try:
        conn   = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.callproc('proc_dezactivare_cont', [int(id_angajat), str(motiv)])
        conn.commit()
        return jsonify({"status": "succes", "mesaj": f"Contul angajatului {id_angajat} dezactivat."}), 200
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        if 'conn' in locals(): conn.close()
    ...
    try:
        ...
        query  = """
            SELECT c.id_concediu, c.id_angajat,
                   ...
                   c.tip, c.data_start, c.data_sfarsit,
                   DATEDIFF(c.data_sfarsit, c.data_start) + 1 AS zile_solicitate,
                   c.id_aprobator, c.status
            FROM concedii c
            JOIN angajati a ON c.id_angajat = a.id_angajat
            WHERE 1=1
        """
        if id_angajat:
            query += " AND c.id_angajat = %s"; params.append(id_angajat)
        query += " ORDER BY c.data_start DESC LIMIT 500"
        cursor.execute(query, tuple(params))
        istoric = cursor.fetchall()
        return jsonify({"status": "succes", "total_cereri": len(istoric), "date_concedii": istoric}), 200
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        if 'conn' in locals() and conn.is_connected():
...
def adauga_concediu():
    ...
    try:
        ...
        if start > sfarsit:
            return jsonify({"status": "eroare", "mesaj": "Data start nu poate fi dupa data sfarsit."}), 400
        ...
        """, (id_angajat, sfarsit, start, start, sfarsit))
        if cursor.fetchone():
            return jsonify({"status": "eroare", "mesaj": "Angajatul are deja un concediu in aceasta perioada."}), 400

        cursor.execute("""
            SELECT m.id_manager FROM manageri m
            JOIN angajati a ON a.id_manager = m.id_manager
            WHERE a.id_angajat = %s
        """, (id_angajat,))
        manager      = cursor.fetchone()
        id_aprobator = manager['id_manager'] if manager else None

        cursor.execute("""
            INSERT INTO concedii (id_angajat, id_aprobator, tip, data_start, data_sfarsit, status)
            VALUES (%s, %s, %s, %s, %s, 'in asteptare')
        """, (id_angajat, id_aprobator, tip_concediu, data_start, data_sfarsit))
        conn.commit()
...
def gestionare_departamente():
    ...
    if request.method == 'GET':
        cursor.execute("SELECT * FROM departamente ORDER BY locatie, nume")
...
def get_sinteza_departamente():
    try:
        ...
        cursor.execute("""
            ...
            FROM departamente d LEFT JOIN angajati a ON d.id_departament = a.id_departament
            GROUP BY d.id_departament
        """)
        departamente = cursor.fetchall()
        for dep in departamente:
            cursor.execute("SELECT id_angajat, nume, prenume, id_pozitie, status FROM angajati WHERE id_departament = %s",
                           (dep['id_departament'],))
            angajati = cursor.fetchall()
            dep['detalii_angajati']      = angajati
            dep['id_pozitii_existente']  = list(set([a['id_pozitie'] for a in angajati if a['id_pozitie']]))
            dep['numar_angajati_activi'] = len([a for a in angajati if a['status'] == 'activ'])
        return jsonify({"status": "succes", "total_departamente": len(departamente), "departamente": departamente}), 200
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        if 'conn' in locals() and conn.is_connected():
...
def gestionare_pozitii():
    ...
    if s_min >= s_max:
        return jsonify({"status": "eroare", "mesaj": "Salariul minim trebuie sa fie mai mic decat cel maxim."}), 400
    try:
        cursor.execute("INSERT INTO pozitii (titlu, id_departament, salariu_min, salariu_max) VALUES (%s, %s, %s, %s)",
                       (date.get('titlu'), date.get('id_departament'), s_min, s_max))
        conn.commit()
        return jsonify({"status": "succes", "mesaj": "Pozitie adaugata"}), 201
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        conn.close()
...
def gestionare_proiecte():
    ...
    try:
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
        """, (date['nume'], date['descriere'], date['data_start'], date['data_sfarsit'], date['status'], date['buget']))
        conn.commit()
        return jsonify({"status": "succes", "mesaj": "Proiect creat"}), 201
    except Exception as e:
        return jsonify({"status": "eroare", "detalii": str(e)}), 400
    finally:
        conn.close()
...
def sterge_alocare_proiect(id_alocare):
    try:
        conn   = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("DELETE FROM alocari_proiecte WHERE id_alocare = %s", (id_alocare,))
        conn.commit()
        if cursor.rowcount == 0:
            return jsonify({"status": "eroare", "mesaj": f"Alocarea {id_alocare} nu a fost gasita."}), 404
        return jsonify({"status": "succes", "mesaj": f"Alocare {id_alocare} stearsa."}), 200
    except mysql.connector.Error as err:
        return jsonify({"status": "eroare_db", "detalii": str(err)}), 500
    finally:
        if 'conn' in locals() and conn.is_connected():
